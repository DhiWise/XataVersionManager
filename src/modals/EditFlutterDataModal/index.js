...
import { updateFlutterData } from "service/api";
...
const EditFlutterDataModalModal = (props) => {
  ...
  const form = useForm(
    { generator_name: props?.record?.generator_name ?? "", version: props?.record?.version ?? "" },
    {
      validate: true,
  ...
  function callApi4(data) {
    const req = { data: { ...data }, id: props?.record?.id };

    updateFlutterData(req)
      .then((res) => {
        setapiData4(res);
      ...
      >
        <div className="m-[auto] max-h-[97vh] overflow-y-auto">
          <Column className="bg-white_A700 justify-end xl:mt-[105px] 2xl:mt-[119px] 3xl:mt-[142px] lg:mt-[92px] lg:p-[12px] xl:p-[14px] 2xl:p-[16px] 3xl:p-[19px] rounded-radius10 shadow-bs w-[100%]">
            ...
            <Input
              className="font-medium p-[0] lg:text-[10px] xl:text-[12px] 2xl:text-[14px] 3xl:text-[16px] placeholder:text-bluegray_900 text-bluegray_900 w-[100%]"
...
export default EditFlutterDataModalModal;
